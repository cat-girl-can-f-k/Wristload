...
final class CentralManagerImpl
  ...
  @override
  void onRfcommData(int addressArgs, Uint8List valueArgs) {
    // RFCOMM 数据由项目层通过 BasicMessageChannel 的 onRfcommData 通道直接
    // 订阅接收（插件的 GATT 事件模型不覆盖经典蓝牙）。
    _logger.info('onRfcommData: $addressArgs - ${valueArgs.length} bytes');
  }
  ...
  Future<List<GATTServiceArgs>> _getServices(
    ...
    for (var serviceArgs in servicesArgs) {
      ...
      try {
        includedServicesArgs = await _getIncludedServices(
          addressArgs,
          handleArgs,
          modeArgs,
        );
      } catch (exception) {
        // Some devices (e.g. Mi Band) reject included-services enumeration with
        // GattCommunicationStatus.InvalidState (status 3). Included services are
        // optional metadata, so degrade gracefully instead of failing discovery.
        _logger.warning(
          'getIncludedServices failed for $addressArgs.$handleArgs, ignored: '
          '$exception',
        );
        includedServicesArgs = const [];
      }
  ...
  Future<List<GATTCharacteristicArgs>> _getCharacteristics(
    ...
    var characteristicsArgs = const <GATTCharacteristicArgs>[];
    Object? lastError;
    var ok = false;
    // Vela wearables (Mi Band 9) may answer status 3 (InvalidState) on the
    // first characteristics query right after connect; retry with backoff.
    for (var attempt = 1; attempt <= 3; attempt++) {
      try {
        characteristicsArgs = await _api.getCharacteristics(
          addressArgs,
          handleArgs,
          modeArgs,
        );
    ...
    if (!ok) {
      _logger.warning(
        'getCharacteristics failed for $addressArgs.$handleArgs after 3 '
        'attempts, degraded to empty: $lastError',
      );
      return const [];
    }
    for (var characteristicArgs in characteristicsArgs) {
      ...
      try {
        descriptorsArgs = await _getDescriptors(
          addressArgs,
          handleArgs,
          modeArgs,
        );
        ...
        _logger.warning(
          'getDescriptors failed for $addressArgs.$handleArgs, ignored: '
          '$exception',
        );
        descriptorsArgs = const [];
      }
