...
namespace {
    ...
    HRESULT DesktopDropTarget::DragEnter(IDataObject *pDataObj, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect) {
        ...
        STGMEDIUM medium;
        if (pDataObj != nullptr &&
            pDataObj->QueryGetData(&format) == S_OK &&
            pDataObj->GetData(&format, &medium) == S_OK) {
            PVOID data = GlobalLock(medium.hGlobal);
            if (data != nullptr) {
                file_count = DragQueryFile(
                        reinterpret_cast<HDROP>(data), 0xFFFFFFFF, nullptr, 0);
        ...
        channel_->InvokeMethod("entered", std::make_unique<flutter::EncodableValue>(
                flutter::EncodableList{
                        ...
                        flutter::EncodableValue(static_cast<int>(file_count))
                }
        ));
        return 0;
    }
