...
class WristloadBackend {
  ...
  MacBluetoothDevice? _device;
  DeviceProfile? _profile;
  BackendConnectionState _connection = BackendConnectionState.disconnected;
  ...
  BackendSnapshot get snapshot => _makeSnapshot();
  List<QueueEntry> get queue => List.unmodifiable(_queue);
  bool get sessionReady => _connection == BackendConnectionState.ready;
  MacBluetoothDevice? get device => _device;
  ...
  void setAuthKey(String value) {
    ...
    if (_connection == BackendConnectionState.awaitingAuthKey) {
      _connection = BackendConnectionState.authenticating;
  ...
  Future<void> clearAuthKey() async {
    ...
    if (_connection != BackendConnectionState.disconnected) {
      _connection = BackendConnectionState.awaitingAuthKey;
    }
    _emit('authkey 与会话材料已从内存清除。');
  }

  Future<void> connect(MacBluetoothDevice device,
    ...
    _connection = BackendConnectionState.connecting;
    ...
    try {
      ...
      // MacBluetoothTransport.connect completes only after a new physical
  ...
  Future<void> disconnect() async {
    if (_disposed && _connection == BackendConnectionState.disconnected) return;
    ...
    _recoveringPostAuthClose = false;
    _postAuthReconnectAttempts = 0;
    _connection = BackendConnectionState.disconnected;
    _device = null;
    _profile = null;
    try {
  ...
  Future<void> _runInstall(InstallRequest request) async {
    ...
    if (request.kind == InstallKind.watchface) {
      ...
      await _clearCheckpoint();
      _publish(
          request, InstallStage.succeeded, '快应用已安装：${parsed.packageName}。',
          successVerifiedByDeviceBusinessEvent: true);
    }
  }
  ...
  void _handlePacket(SppPacket packet) {
    if (packet.type == SppProtocol.typeCmd &&
      ...
      if (_resumeAuthenticatedSession && _cipher != null) {
        ...
        _connection = BackendConnectionState.ready;
        _watchdog?.cancel();
  ...
  Future<void> _sendAuthConfirm(List<int> watchNonce, List<int> watchHmac,
    ...
    try {
      await _transport
          .write(SppProtocol.buildDataFrame(sequence, command));
    } on Object {
      _discardAck(ack);
      rethrow;
    }
  ...
  BackendSnapshot _makeSnapshot() => BackendSnapshot(
