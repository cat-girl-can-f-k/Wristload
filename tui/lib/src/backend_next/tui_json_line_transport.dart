...
final class TuiJsonLineMacBluetoothTransport
  ...
  Future<void> connect(
    ...
    try {
      ...
      if (error is TuiNativeTransportException) {
        try {
  ...
  void _handleLine(int epoch, String line) {
    ...
    if (requestId is String) {
      ...
      if (pending != null) {
        if (eventName == 'error') {
          ...
          requestHandled = true;
          pending.completer
              .completeError(TuiNativeTransportException.fromJson(event));
    ...
    switch (eventName) {
      ...
      case 'error':
        if (!requestHandled) {
          _errors.add(TuiNativeTransportException.fromJson(event));
        }
        return;
      default:
        return;
