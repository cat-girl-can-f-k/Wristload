...
class HciDecoder {
  ...
  HciDecodeReport decode({
    ...
    int inboundCounterStart = 0,
    int inboundCounterWindow = 4096,
  }) {
    final secret = XiaomiAuth.secretKeyFromHex(authKeyHex);
    if (secret == null) throw const FormatException('authkey 必须为 32 位十六进制');
    if (inboundCounterStart < 0 || inboundCounterWindow < 1) {
      throw const FormatException('计数器范围无效');
    }
    ...
    for (final frame in l1) {
      if (frame.type != SppProtocol.typeData || frame.payload.length < 2) continue;
      ...
      if (opCode == SppProtocol.opCodeWrite) {
        ...
        if (frame.outbound && command?.type == XiaomiAuth.commandType &&
            command?.subtype == XiaomiAuth.cmdNonce && command?.appNonce != null) {
          appNonce = command!.appNonce;
          watchNonce = null;
          cipher = null;
          lines.add('发现 f=26 请求：等待同会话设备随机数。');
        } else if (!frame.outbound && command?.type == XiaomiAuth.commandType &&
            command?.subtype == XiaomiAuth.cmdNonce && command?.watchNonce != null && appNonce != null) {
          watchNonce = command!.watchNonce;
          lines.add('发现 f=26 响应：设备随机数已匹配。');
        } else if (!frame.outbound && command?.type == XiaomiAuth.commandType &&
            command?.subtype == XiaomiAuth.cmdAuth && command?.authStatus == 1 &&
            appNonce != null && watchNonce != null) {
      ...
      final hit = _decryptInstallCommand(
        ...
        start: inboundCounterStart,
        length: inboundCounterWindow,
      );
      if (hit != null) {
        final direction = frame.outbound ? '手机→设备' : '设备→手机';
        lines.add('安装业务命中：$direction，L1 seq=${frame.seq}，'
            '${frame.outbound ? 'appCounter' : 'deviceCounter'}=${hit.counter}，'
            'command=${hit.command.type}/${hit.command.subtype}，${data.length}B。');
      }
    }
    ...
    if (encrypted == 0) {
      ...
      lines.add('发现 $encrypted 个 WRITE_ENC 帧，但计数器范围内没有严格的安装业务命中。');
    }
    return HciDecodeReport(l1Frames: l1.length, sessions: sessions, encryptedFrames: encrypted, lines: lines);
  }

  _DecryptHit? _decryptInstallCommand(SessionCipher cipher, List<int> encrypted,
      {required bool outbound, required int start, required int length}) {
    for (var counter = start; counter < start + length; counter++) {
      final plaintext = outbound
          ? cipher.encryptOutbound(encrypted, counter: counter) // CTR 解密=加密
          : cipher.decryptInbound(encrypted, counter: counter);
      final command = XiaomiAuth.parse(plaintext);
      if (command != null && _isInstallCommand(command)) return _DecryptHit(counter, command);
    }
    return null;
  }
  ...
  List<_L1Frame> _extractL1Frames(Uint8List bytes) {
    ...
    for (final record in records) {
      ...
      if (pb != 1) {
        ...
        pending[key] = _L2Pending(record.outbound, _le16(acl, 2), length, acl.sublist(4));
      } else {
  ...
  void _consumeRfcomm(_L2Pending pdu, Map<String, List<int>> streams, List<_L1Frame> frames) {
    ...
    for (var offset = 0; offset + 4 <= bytes.length;) {
      ...
      final length = header == 3 ? firstLength >> 1 : (firstLength >> 1) | (bytes[offset + 3] << 7);
  ...
  void _consumeL1Stream(List<int> stream, bool outbound, List<_L1Frame> frames) {
    for (var offset = 0; offset + SppProtocol.headerSize <= stream.length;) {
      ...
      if (computeCrc16(payload) == crc) {
        frames.add(_L1Frame(stream[offset + 2] & 0x0f, stream[offset + 3], payload, outbound));
      }
      offset = end;
    }
  ...
  List<_SnoopRecord> _btsnoopRecords(Uint8List bytes) {
    ...
    for (var offset = 16; offset + 24 <= bytes.length;) {
      ...
      if (original != included || included < 3 || included > 70000 || flags > 3 || offset + 24 + included > bytes.length) {
        offset++;
        continue;
      }
  ...
  int _le16(List<int> data, int offset) => data[offset] | (data[offset + 1] << 8);
  int _be32(List<int> data, int offset) =>
      (data[offset] << 24) | (data[offset + 1] << 16) | (data[offset + 2] << 8) | data[offset + 3];
}

class _SnoopRecord { const _SnoopRecord(this.outbound, this.data); final bool outbound; final List<int> data; }
class _L2Pending { _L2Pending(this.outbound, this.cid, this.length, List<int> first) : data = [...first]; final bool outbound; final int cid; final int length; final List<int> data; }
class _L1Frame { const _L1Frame(this.type, this.seq, this.payload, this.outbound); final int type; final int seq; final List<int> payload; final bool outbound; }
class _DecryptHit { const _DecryptHit(this.counter, this.command); final int counter; final ParsedAuthCommand command; }
