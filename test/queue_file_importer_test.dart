...
void main() {
  ...
  test('macOS rejects every supported source without a bookmark', () async {
    ...
    expect(result.failures.map((failure) => failure.error),
        everyElement(isA<StateError>()));
    expect(metadataReads, 0);
  });

  test('macOS counts an unsupported path without requesting file access',
      () async {
    debugDefaultTargetPlatformOverride = TargetPlatform.macOS;
    ...
    final result = await QueueFileImporter().prepare(['/tmp/firmware.zip']);
  ...
  test('macOS checks a bookmarked file type after resolving its stale path',
      () async {
    debugDefaultTargetPlatformOverride = TargetPlatform.macOS;
    final methods = <String>[];
    messenger.setMockMethodCallHandler(channel, (call) async {
  ...
  test('macOS single-file request keeps the resolved path when queued',
      () async {
    debugDefaultTargetPlatformOverride = TargetPlatform.macOS;
  ...
  test('single-file import rejects a resolved file of another install kind',
      () async {
    debugDefaultTargetPlatformOverride = TargetPlatform.macOS;
    final methods = <String>[];
    messenger.setMockMethodCallHandler(channel, (call) async {
