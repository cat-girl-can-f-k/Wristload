...
void main() {
  testWidgets(
    'authkey editor keeps normal selection behavior and saves replacement',
    (tester) async {
  ...
  testWidgets(
    'history binding picker keeps selected device until modification',
    (tester) async {
      final bindings = [
        AuthKeyBinding(
          id: 'device-a',
              ...
              body: TextButton(
                onPressed: () {
                  result = showDialog<AuthKeyBinding>(
                    ...
                    barrierDismissible: false,
                    builder: (_) =>
                        buildAuthKeyBindingPickerForTesting(bindings: bindings),
                  );
                },
      ...
      final radio = tester.widget<Radio<AuthKeyBinding>>(
        find.byWidgetPredicate(
          (widget) =>
              widget is Radio<AuthKeyBinding> && widget.value?.id == 'device-b',
        ),
      );
  ...
  testWidgets('history binding picker removes a deleted saved device', (
    tester,
  ) async {
    final bindings = [
      AuthKeyBinding(
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Builder(
          builder: (context) => Scaffold(
            body: TextButton(
              onPressed: () => showDialog<AuthKeyBinding>(
                context: context,
                barrierDismissible: false,
                builder: (_) => buildAuthKeyBindingPickerForTesting(
                  bindings: bindings,
                  onDelete: (binding) async {
                    deletedIds.add(binding.id);
                    return true;
                  },
                ),
              ),
    ...
    await tester.tap(find.text('设备 B'));
    await tester.tap(
      find.byKey(const ValueKey('delete-saved-binding-device-b')),
    );
    await tester.pumpAndSettle();
    ...
    expect(deletedIds, ['device-b']);
    expect(find.text('设备 B'), findsNothing);
    final modifyButton = tester.widget<FilledButton>(
      find.widgetWithText(FilledButton, '修改'),
    );
    expect(modifyButton.onPressed, isNull);
  });
}
